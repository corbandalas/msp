...
public class BankStatementTriggerJob implements Runnable {
    ...
    public void run() {
        ...
        F.Promise.wrap(customerRepository.retrieveAll()).map(res -> {
            res.parallelStream().forEach(customer -> mailService.sendBankStatementEmail("noreply@mysafepay.dk", customer.getEmail(), customer.getCountry_id(), customer.getFirstName() + " " + customer.getLastName()));
