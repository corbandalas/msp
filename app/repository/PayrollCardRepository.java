...
public class PayrollCardRepository implements BaseCRUDRepository<PayrollCard> {
        ...
        connectionPool.getConnection().query("select nextval('" + connectionPool.getSchemaName() + ".payroll_card_seq')",
                idResult -> {
                    ...
                    final String query = "INSERT INTO " + connectionPool.getSchemaName() + ".payroll_card (id, payrollrequestid," +
                            "accno, pubtoken, pan, title, lastname, firstname, dob, email, mobtel, addrl1, addrl2, addrl3, city, postcode, country, amount, currency, cardname, expdate, cvv, islive, payrollcardstatus) VALUES ($1, $2, $3, $4, $5, $6, " +
                    ...
                    connectionPool.getConnection().query(query, asList(id, entity.getPayrollRequestId(), entity.getAccno(), entity.getPubtoken(), entity.getPan(), entity.getTitle(), entity.getLastName(), entity.getFirstName(), entity.getDob(), entity.getEmail(), entity.getMobtel(),
                            entity.getAddrl1(), entity.getAddrl2(), entity.getAddrl3(), entity.getCity(), entity.getPostcode(), entity.getCountry(), entity.getAmount(), entity.getCurrency(), entity.getCardName(), entity.getExpDate(), entity.getCvv(), entity.getIsLive(), entity.getPayrollCardStatus().name()),
                            result -> {
    ...
    public Future<PayrollCard> update(PayrollCard entity) {
        ...
        final String query = "UPDATE " + connectionPool.getSchemaName() + ".payroll_card SET pan=$2, pubtoken=$3," +
                " pan=$4, cardname=$5, expdate=$6, cvv=$7, payrollcardstatus=$8 WHERE id=$1";
        connectionPool.getConnection().query(query, asList(entity.getId(), entity.getPan(), entity.getPubtoken(), entity.getPan(), entity.getCardName(), entity.getExpDate(), entity.getCvv(),
    ...
    public PayrollCard createEntity(Row row) {
        return new PayrollCard(row.getLong("id"), row.getLong("payrollrequestid"), row.getString("accno"), row.getString("pubtoken"), row.getString("pan"), row.getString("title"), row.getString("lastname"), row.getString("firstname"), row.getString("dob"), row.getString("email"), row.getString("mobtel"), row.getString("addrl1"), row.getString("addrl2"), row.getString("addrl3"), row.getString("city"), row.getString("postcode"), row.getString("country"), row.getString("amount"), row.getString("currency"), row.getString("cardname"), row.getString("expdate"), row.getString("cvv"), row.getString("islive"), PayrollCardStatus.valueOf(row.getString("payrollcardstatus")));
    }
